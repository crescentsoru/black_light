...
#soru-
#Stole this from Nyx's project, who likely stole it from this pastebin, the originator of which I can't find anymore:
#https://pastebin.com/6T82HgW2
...
const actions = ["up","down","left","right","jump","attackA","attackB","attackC", \
 "dodge","grab","cstickup","cstickdown","cstickleft","cstickright"] #soru-original names for actions without the p1_ p2_ etc prefixes

var INPUT_ACTIONS = [ "p1_up","p1_down","p1_left","p1_right","p1_jump","p1_attackA","p1_attackB","p1_attackC", \
"p1_dodge", "p1_grab", "p1_cstickup", "p1_cstickdown", "p1_cstickleft", "p1_cstickright"] #soru- the current inputs being changed at the moment




const CONFIG_FILE = "res://Configs/input.cfg"
...
func switch_player(playerindex): #soru- for switching which player port you're editing controls of.
	var old_buttons = INPUT_ACTIONS
	INPUT_ACTIONS = []
	for x in actions:
		var newstring = "p" + str(playerindex) + "_" + x
		INPUT_ACTIONS.append(newstring)
	for index in len(old_buttons):
		for node in get_node("bindings").get_children(): #yeah this is expensive but idc
			if node.name == old_buttons[index]:
				node.name = INPUT_ACTIONS[index]
	initialize_buttons()
	load_config()
...
func initialize_buttons(): #soru- refactored cause it is ran again when you switch players
	for action in INPUT_ACTIONS:
		...
		if len(InputMap.get_action_list(action)) > 0:
			...
			button.text =  'Empty'
		button.disconnect("pressed", self, "wait_for_input") #soru addition because im clueless
